...
namespace Knife.Effects.SimpleController
{
    public class GravityGunWeapon : MonoBehaviour, IWeapon
    {
        ...
        {
            currentFov = playerCamera != null ? playerCamera.fieldOfView : DefaultFov;
            OnEnableHook();
        }

        private void OnDisable()
        {
            ReleaseGrabbedBody(restoreVelocity: false);
        ...
        private void FixedUpdate()
        {
            if (isGrabbed)
            {
                if (grabbedBody == null || attractPoint == null)
                {
                    ReleaseGrabbedBody(restoreVelocity: false);
                    return;
                }
        ...
        protected void Leave()
        {
            ...
            if (isGrabbed)
            {
                ReleaseGrabbedBody(restoreVelocity: true);
            }
        ...
        private void Throw()
        {
            ...
            if (isAttached && grabbedBody != null)
            {
                ...
                if (playerCamera == null || attractPoint == null)
                {
                    ReleaseGrabbedBody(restoreVelocity: false);
                    return;
                }
        ...
        private void ReleaseGrabbedBody(bool restoreVelocity)
        {
            if (grabbedBody != null)
            {
                grabbedBody.transform.SetParent(grabbedBodyParent);
                ...
                if (restoreVelocity)
                {
                    Vector3 velocity = grabbedBody.velocity;
                    if (grabbedBodyPositions.Count > 1)
                    {
                        ...
                        velocity /= (grabbedBodyPositions.Count - 1);
                    }
            ...
            grabbedBody = null;
            grabbedBodyColliders = null;
            grabbedBodyParent = null;
            isGrabbed = false;
            isAttached = false;
        }
    }
